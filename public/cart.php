...
$sql = "SELECT products.price AS Price, products.id, products.name AS Name 
        FROM users_products 
...
if ($stmt->rowCount() >= 1) {
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $sum += $row["Price"];
        $id = $row["id"] . ", ";
        echo "<tr><td>" . "#" . $row["id"] . "</td><td>" . $row["Name"] . "</td><td> " . $row["Price"] . "đ</td><td><a href='cart-remove.php?id={$row['id']}' class='remove_item_link'> Remove</a></td></tr>";
    }
    $id = rtrim($id, ", ");
    echo "<tr><td></td><td>Total</td><td> " . $sum . "đ</td><td><a href='success.php' class='btn btn-primary'>Confirm Order</a></td></tr>";
    ?>
    </tbody>
    <?php
} else {
